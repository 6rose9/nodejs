...
// html -> form (default = URL encorded format) -> server (need to decode to understand)
// eg. encoded name=su%su&age=20&city="yangon"
// extended: ture (nested objects or arrays support)
...
app.use((req, res, next) => {
    if (mongoose.connection.readyState !== 1) {
        return res.status(503).send('Database is not connected. Please try again later');
    }
    next();
});
